function init() {
  ...
  for (i = 0; i < LR.images.length; i++) {
    ...
    LR.images[i].$thumbnail = $(
      '<div class="thumbnail" data-large-img="images/large/' +
        LR.images[i].exportFilename +
        '.jpg" data-id="ID' +
        LR.images[i].id +
        '" data-title="' +
        LR.images[i].title +
    ...
    LR.images[i].$thumbnailImg.on("load", onThumbnailImgLoad);
    LR.images[i].$thumbnailImg.on("error", onThumbnailImgError);
  ...
  function renderAllThumbnails() {
    for (var i = 0; i < LR.images.length; i++) {
      ...
      LR.images[i].$thumbnail.on("click", onThumbnailClick);
  ...
  function initLoadOnScroll() {
    ...
    LR.images[0].$thumbnail.on("click", onThumbnailClick);
    ...
    var _thumbsPerRow = Math.ceil(
      (_availableWidth + _scrollbarWidth) / _thumbOuterWidth
    );
    ...
    _$w.on("scroll", onWindowLoadScroll);
  }
  ...
  function checkForSpace() {
    ...
    if (_lastThumbLeftOffset + _thumbWidth < _viewportWidth) {
      _extraItemsToLoad =
        (_viewportWidth - (_lastThumbLeftOffset + _thumbWidth)) / _thumbWidth;
    ...
    if (
      _$w.scrollTop() + _viewportHeight == _$body.height() &&
      _thumbsToLoad == 0 &&
      _lastLoadedThumbIndex < LR.images.length - 1
    ) {
      loadMoreThumbnails(
        _lastLoadedThumbIndex + 1,
        getCurrentColumnCount() * 2 + _extraItemsToLoad
      );
  ...
  _$buttonClose.on("click", closeLoupeView);

  _$buttonPrev.on("click", showPrevImage);

  _$buttonNext.on("click", showNextImage);

  _$hotspotPrevLoupe.on("mouseover", onHotspotPrevLoupeOver);

  _$hotspotPrevLoupe.on("mouseout", onHotspotPrevLoupeOut);

  _$hotspotPrevLoupe.on("click", showPrevImage);

  _$hotspotNextLoupe.on("mouseover", onHotspotNextLoupeOver);

  _$hotspotNextLoupe.on("mouseout", onHotspotNextLoupeOut);

  _$hotspotNextLoupe.on("click", showNextImage);
  ...
  function openLoupeView(snap) {
    ...
    $(document).on("keydown", onLoupeKeyDown);
  }
  ...
  function loadImageForThumbnail($thumbnail) {
    ...
    $("<img/>")
      .css("opacity", 0)
      .attr("src", $thumbnail.attr("data-large-img"))
      .load(function () {
        $(this).remove();
        setImage();
      });
    ...
    if ($thumbnail.attr("data-caption") != "nil") {
      _metadata +=
        '<p class="caption">' + $thumbnail.attr("data-caption") + "</p>";
    }
  ...
  function setImage() {
    ...
    $(window).on("resize", onLoupeResize);
  }
  ...
  function hideCurrentImage() {
    ...
    $(window).off("resize", onLoupeResize);
  }
  ...
  function closeLoupeView(e) {
    ...
    _$loupeContainer.fadeOut(0);
    _$loupeImage.remove();
    $(document).off("keydown", onLoupeKeyDown);
    unlockBody();
  ...
  if (Modernizr.fullscreen) {
    $("#buttonFullscreen").on("click", toggleFullScreen);
  }
